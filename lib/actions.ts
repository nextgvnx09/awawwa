...
export const fetchNewsRealtime = (callback: (news: NewsItem[]) => void) => {
  // Jika Supabase tidak dikonfigurasi, gunakan localStorage dengan polling
  if (!isSupabaseConfigured()) {
    const initialNews = getStoredNews();
    callback(initialNews);
    
    // Polling setiap 5 detik untuk simulasi realtime
    const interval = setInterval(() => {
      const updated = getStoredNews();
      callback(updated);
    }, 5000);
...
export const insertNewsSupabase = async (news: Omit<NewsItem, 'id' | 'date' | 'created_at'>) => {
  if (!isSupabaseConfigured()) {
...
export const updateNewsSupabase = async (id: string | number, news: Omit<NewsItem, 'id' | 'date'>) => {
  if (!isSupabaseConfigured()) {
    return updateNews(id, news);
  }
...
export const deleteNewsSupabase = async (id: string | number) => {
  if (!isSupabaseConfigured()) {
